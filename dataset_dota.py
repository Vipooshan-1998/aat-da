...
class CrossValDataset(Dataset):
    def __init__(self, dataset_path, img_dataset_path, toas_files_path, split_path, ref_interval, objmap_file, training, ego_dist, n_frames, attention_path):
    ...
    def __getitem__(self, index):
        ...
        # Attention
        if curr_vid_label > 0:
            att_file = os.path.join(self.attention_path, feature_path.split('/')[-1].split(".")[1], "positive",
                                    feature_path.split('/')[-1].split(".")[0] + '.mp4')    # mp4 # avi
        else:
            att_file = os.path.join(self.attention_path, feature_path.split('/')[-1].split(".")[1], "negative",
                                    feature_path.split('/')[-1].split(".")[0] + '.mp4')   # mp4 # avi
